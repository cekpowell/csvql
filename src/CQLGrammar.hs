...
data HappyAbsSyn t4
	= HappyTerminal (CQLToken)
	| HappyErrorToken Prelude.Int
	| HappyAbsSyn4 t4

happyExpList :: Happy_Data_Array.Array Prelude.Int Prelude.Int
happyExpList = Happy_Data_Array.listArray (0,15) ([32,32769,0,2052,512,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Exp","READ","LET","'='","Filename","Str","Var","%eof"]
...
happyReduction_1 ((HappyTerminal (TokenFilename _ happy_var_5)) `HappyStk`
	_ `HappyStk`
	_ `HappyStk`
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenRead _ -> cont 5;
	TokenLet _ -> cont 6;
	TokenEquals _ -> cont 7;
	TokenFilename _ happy_dollar_dollar -> cont 8;
	TokenStr _ happy_dollar_dollar -> cont 9;
...
happyError' :: () => ([(CQLToken)], [Prelude.String]) -> HappyIdentity a
